-- Script de déploiement simplifié pour le mode manuel par dossier
-- Exécutez ce script dans l'éditeur SQL de Supabase

-- ========================================
-- ÉTAPE 1: CORRECTION PRÉALABLE
-- ========================================

-- Supprimer la contrainte de clé étrangère sur master_id si elle existe
DO $$
BEGIN
  IF EXISTS (
...
-- ========================================
-- ÉTAPE 2: MIGRATION PRINCIPALE
-- ========================================

-- Ajouter les colonnes master_id et master_name à la table invoices
...
-- Table pour gérer les modes des dossiers
...
-- Fonction helper pour vérifier le mode d'un dossier
...
-- Fonction pour activer/désactiver le mode manuel d'un dossier
...
-- Fonction pour obtenir le mode d'un dossier depuis un payment_id
...
-- Vue dossiers avec leur mode de gestion
...
-- Vue paiements avec contexte dossier
...
-- Fonction pour créer un paiement en tenant compte du mode du dossier
...
-- Modifier le trigger d'allocation automatique pour vérifier le mode du dossier
...
SELECT 'Colonnes master_id et master_name ajoutées' as status,
       EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'invoices' AND column_name = 'master_id') as master_id_exists,
       EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'invoices' AND column_name = 'master_name') as master_name_exists;

-- Vérifier que la table dossier_settings existe
SELECT 'Table dossier_settings créée' as status,
       EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'dossier_settings') as table_exists;

-- Vérifier que la fonction existe
SELECT 'Fonction get_dossier_manual_mode créée' as status,
       EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_name = 'get_dossier_manual_mode') as function_exists;

-- Vérifier que la vue existe
SELECT 'Vue dossier_overview_with_mode créée' as status,
       EXISTS (SELECT 1 FROM information_schema.views WHERE table_name = 'dossier_overview_with_mode') as view_exists;

-- Test simple de la fonction
SELECT 'Test de la fonction' as status,
       get_dossier_manual_mode('DOSSIER-TEST') as result;

-- Afficher le nombre de dossiers créés
SELECT 'Nombre de dossiers créés' as status,

...
-- SCRIPT 1 : Suppression des anciennes fonctions et vues
...
DROP VIEW IF EXISTS master_mode_status;
...
BEGIN
  ...
  UPDATE "MASTER"
  SET 
    "FACTURE" = 'famille',                    -- Automatique en mode manuel
    "REGLEMENT" = reglement_statut,           -- Choix utilisateur
    "MANUAL_OVERRIDE" = TRUE
  WHERE "DOSSIER" = dossier_id;
  
  RETURN 'Mode manuel active pour: ' || dossier_id || 
         ' (facture: famille, reglement: ' || reglement_statut || ')';
END;
$$ LANGUAGE plpgsql;
